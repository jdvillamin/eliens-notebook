...
def generateString(basepath):
    ...
    # directories
    for dir in dirs:
        res += "[" + " ".join([word[0:1].upper() + word[1:]
                              for word in dir.split("_")]) + "]\n"
        ...
        # files
        for file in files:
            res += dir + "/" + file + '\t' + " ".join([word[0:1].upper() + word[1:] for word in re.sub(
                r'\.{}'.format(basepath if basepath == "cpp" else 'py'), '', file).split("_")]) + '\n'
